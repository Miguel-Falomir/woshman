...
public class DAO_Empleado extends DAO implements DAO_Interface<Empleado, Integer> {
    ...
    public List<Empleado> searchMecanicos(){
        // variables internas
        PreparedStatement statement = null;
        ResultSet resultado_1 = null;
        ResultSet resultado_2 = null;
        ...
        try {
            // consulta 1: buscar todos los datos de rol y empleado
            statement = connect.prepareStatement("SELECT e.id_empleado, e.nombre, e.apellidos, e.username, e.password, r.id_rol, r.nombre, r.descripcion FROM empleado e JOIN rol r ON e.fk_rol = r.id_rol WHERE r.id_rol = 1;", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ...
            while (resultado_1.next()){
                ...
                // consulta 3: contar averias asignadas al empleado
                statement = connect.prepareStatement("SELECT count(*) FROM averia ave WHERE ave.fk_empleado = ?;", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
                statement.setInt(1, auxEmpleado.getId());

                // ejecutar consulta
                resultado_2 = statement.executeQuery();

                // asegurar que 'resultado_2' apunte a la primera fila
                if (!resultado_2.isBeforeFirst()){
                    resultado_2.beforeFirst();
                }
                resultado_2.next();
                ...
                countAverias = resultado_2.getInt(1);
                auxEmpleado.setCantAverias(countAverias);
